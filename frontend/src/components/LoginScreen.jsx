...
export default function LoginScreen() {
  ...
  useEffect(() => {
    let m = true;
    getGoogleAuthConfig()
      .then((c) => { if (m) setGoogleConfigured(!!c?.configured); })
      .catch(() => { if (m) setGoogleConfigured(false); });
    return () => { m = false; };
  }, []);
  ...
  return (
    <div className="min-h-screen w-full flex items-center justify-center p-6 relative overflow-hidden bg-gradient-to-br from-slate-50 via-white to-indigo-50">
      ...
      <div className="relative z-10 w-full max-w-md bg-white/80 backdrop-blur-xl border border-white/60 shadow-2xl rounded-3xl p-8">
        ...
        {/* Google button (real Google OAuth via @react-oauth/google) */}
        {googleConfigured ? (
          <div className="w-full flex justify-center" data-testid="google-login-btn">
            <GoogleLogin
          ...
          <div
            data-testid="google-login-unconfigured"
            className="w-full text-center text-[11px] text-slate-500 bg-slate-50 border border-slate-200 rounded-2xl px-4 py-3"
          >
            Google Sign-In no configurado. Ve a <b>Base de datos → Soporte avanzado → Google Sign-In</b> y guarda tu Client ID.
          </div>
        )}
